...
```
https://snowsurfstudio.net/admin/
```
...
1. 打開 `https://snowsurfstudio.net/admin/`
...
- **預約行事曆**：像 Google 日曆一樣一格一格的月曆，點一下日期就把那天標成「已被預約走」（紅色），首頁的預約表單上那天就會顯示額滿、不能選。這一格跟「預約管理」的 Google 表單資料是分開的兩份東西——封不封鎖完全你自己決定，不需要真的有人預約，是拿來營造熱門搶手感的（飢餓行銷）。再點一次同一天就會解除封鎖、重新開放。
