...
| 2026-09-19 | CloudAD〈GA4 × Meta〉講義重點整理並對照網站實測：發現 GTM 容器（第 8 版）沒有任何網站自訂事件的觸發條件、Google Ads LINE 轉換標記不存在、預約 GA Client ID 永遠空白、成交回傳 GA4 的程式不存在；修正追蹤並開 PR，寫 UTM 規範，Dashboard 加數據優化路線圖 | 數據分析（主責）＋行銷（網站實作）；主線直接處理，未派 Agent | `13_SOP_Workflows/utm-guidelines.md`；`DASHBOARD.md`「數據優化路線圖」；`automation/gas/ga4-conversions.gs` |
| 2026-09-25 | GA4 LINE 點擊追蹤檢查：GA4 收不到 `click_line` 是因為網站送的是 `line_click`；改送 `click_line`（含 link_url／page_path／page_title），補 line.me 判斷與同頁跳轉等待；查出 GA4 的 `ads_conversion___2` 是 GA4 後台「建立事件」規則（`/plans/companion-skiing` 的 page_view），不是 LINE 點擊、與 Meta 無關，未刪除 | 數據分析（主責）＋行銷（網站實作）；主線直接處理，未派 Agent | `approval-queue.md` SSS-20260925-043 |
| 2026-09-25 | LINE 點擊追蹤依 Ellie 指示改回統一的 GA4 `line_click`，加 `button_location`；盤點全站 91 個 LINE 連結／按鈕（29 頁，全部是 lin.ee/xcqRIdT） | 數據分析（主責）＋行銷（網站實作）；主線直接處理，未派 Agent | `approval-queue.md` SSS-20260925-044 |
