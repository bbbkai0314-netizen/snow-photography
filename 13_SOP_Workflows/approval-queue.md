...
| --- | --- | --- | --- | --- | --- | --- |
| SSS-20260925-044 | Website / Tracking | LINE 點擊統一成 GA4 `line_click`（停用上一版的 `click_line`），參數 `page_path`、`page_title`、`link_url`、`button_location`（header／footer／floating_button／blog_content／life_chapter_carousel／booking_confirmation／page_content）；dataLayer 每次點擊只有一筆 line_click | 只改 `src/js/tracking.js`；不動 GTM、Meta Pixel、Meta Lead 的 content_name（Add_To_Line）、Google Ads LINE 轉換、視覺與文案；本機建置＋Playwright 桌機／手機實測首頁、家庭滑雪文章、陪伴式滑雪方案頁各點一次都只有 1 筆 line_click、無 JS error，測試未送出真實資料 | APPROVED | 2026-09-25 Ellie 指示「把所有 LINE 點擊追蹤統一成同一個 GA4 事件：line_click」，同意主線修改並開 PR。同日 Ellie 回覆「核准」，核准 PR #57 合併上線 | PR #57 合併上線；請在 GA4 即時總覽驗證 line_click |
| SSS-20260925-043 | Website / Tracking | GA4 LINE 點擊事件改名為 `click_line`（取代原本送 GA4 的 `line_click`，避免重複），帶 `link_url`、`page_path`、`page_title`、`cta_source`；LINE 判斷補上 line.me／liff.line.me／line://；同頁跳轉的 LINE 連結（文章內文、人生章節輪播）等 GA4 回呼最多 300ms 再開 LINE；800ms 內重複點擊只算一次 | 只改 `src/js/tracking.js`、`src/js/script.js`；不重裝 GA4、不動 GTM／Meta Pixel；Google Ads LINE 轉換與 Meta `Lead`（Add_To_Line 來源）照舊；不改視覺、排版、文案；本機建置＋Playwright 桌機／手機實測（首頁、家庭滑雪文章、陪伴式滑雪方案頁）各送一次 click_line、無 JS error，測試未送出任何真實資料 | APPROVED | 2026-09-25 Ellie 指示檢查並修正 GA4 click_line 追蹤、先檢查 ads_conversion__2 不刪除；同意主線修改並開 PR。同日 Ellie 回覆「核准。」核准 PR #56 合併上線 | PR #56 合併上線；上線後請在 GA4 DebugView 驗證 |
| SSS-20260920-042 | Website | 〈一張好看的滑雪照，是怎麼拍出來的？〉頁面兩個表格排版修正：`src/posts/how-to-photograph-ski-action.md` 的「如果第一趟沒拍到」與「出發前的五項檢查」加品牌表格樣式、固定欄寬與手機換行；同步把五處誤顯的 Markdown 粗體符號轉為正常粗體，並移除 Ellie 指定的「滑過來，我幫你拍」整段；長文來源 SHA-256 `eb938e9580bd0c3f92481a959104373dd21decd9fe872d27a5eafbd6b6437b94` | 只改文章 HTML 呈現，除 Ellie 指定刪除的整段外，不改其餘正文、圖片、SEO 或 CTA；本機 Eleventy 建置通過，待正式頁面檢查 | APPROVED | 2026-09-20 Ellie 提供正式頁截圖並指示：「這裡你要做表格，因為你的字都歪歪的。」並補充「這一句也拿掉」，核准該頁的表格排版與指定刪文 | 待推送與正式頁面驗證 |
| SSS-20260920-041 | Website | 9/21 B2 長文〈一張好看的滑雪照，是怎麼拍出來的？拍攝前先安排好這五件事〉；網站稿 `src/posts/how-to-photograph-ski-action.md`，來源 `04_Content/drafts/2026-09-21-b2-ski-photo/article.md` SHA-256 `63256ee0bd2ded71a52683b4009696db5ed0b6b88cbe3e6e67c29e1cbd9375af`；僅網站文章，不含 FB／IG／Threads | Nikon 與 FIS 原始來源已核對；網站模板轉換、SEO、圖片載入與本機建置通過；採網站既有雪道照片，正文不宣稱品牌實拍案例；先前獨特性 8／10 的缺口仍存在，由 Ellie 決定先上線此版 | APPROVED | 2026-09-20 Ellie：「我改好了，你確認一下版本，就可以上線我的網站。」後補充「我重新更新了文章。以這個內容為主」。核准以上述最新檔案為準的網站文章上線，不含社群發布 | 2026-09-20 PR #49 已合併（merge commit `24a753a`）；GitHub Pages run `35497163900` 成功。正式文章、圖片、Journal 與首頁均回傳 200；確認最新標題、五步驟、CTA 上線且內部編輯備註未公開 |
