 ...
 * 也直接呼叫該檔案的 handleMarkerEdit_()，讓後台存檔跟直接在試算表打勾觸發同一套
 * 收訖信 + GA4 booking_confirmed/purchase 事件邏輯。
 *
...
function doPost(e) {
  try {
    ...
    if (body.action === "update") {
      ...
      // 跟直接在 Google Sheet 打勾走同一套邏輯（定義在 booking-confirmation.gs，
      // 兩份程式碼要貼在同一個 Apps Script 專案），這樣不管是這裡存檔還是直接
      // 在試算表打勾，訂金/尾款收訖信跟 GA4 booking_confirmed/purchase 事件都會觸發。
      // handleMarkerEdit_ 內部本來就有「已經送過就跳過」的判斷，所以重複呼叫沒問題。
      const headerRow = sheet.getRange(1, 1, 1, sheet.getLastColumn()).getValues()[0];
      handleMarkerEdit_(sheet, body.row, headerRow, MARKER_COLUMNS.DEPOSIT_RECEIVED, NOTIFY_STATUS_COLUMNS.DEPOSIT_EMAIL, sendDepositReceivedEmail_, GA_EVENTS.BOOKING_CONFIRMED);
      handleMarkerEdit_(sheet, body.row, headerRow, MARKER_COLUMNS.FINAL_PAYMENT_RECEIVED, NOTIFY_STATUS_COLUMNS.FINAL_EMAIL, sendFinalPaymentReceivedEmail_, GA_EVENTS.PURCHASE);
      return jsonOutput_({ ok: true, rows: readAllRows_(sheet) });
    }
